...
export const StepTwo = () => {
  ...
  const _mergeValues = (values: TestCenter) => {
    const newState = produce(testCenter, (draft) => {
      draft.primaryCoordinator = {
        ...values.primaryCoordinator,
        address: draft.primaryCoordinator.address,
      };
    });
  ...
  return (
    <div className="mx-auto w-6/12 mb-16">
      <h1 className="text-center">Primary Coordinator</h1>
      <FormProvider {...form}>
        <form className="space-y-4" onSubmit={submitHandler} noValidate>
          <PersonForm whichPerson="primaryCoordinator" />
