...
const StepFiveSchema = object({
  techCoordinator: object({
...
export const StepFive = () => {
  ...
  const _mergeValues = (values: TestCenter) => {
    const newState = produce(testCenter, (draft) => {
      draft.techCoordinator = {
        ...values.techCoordinator,
      };
    });
  ...
  return (
    <div className="mx-auto w-6/12 mb-16">
      <h1 className="text-center">Tech Coordinator</h1>
      <FormProvider {...form}>
        <form className="space-y-4" onSubmit={submitHandler} noValidate>
          <PersonForm whichPerson="techCoordinator" />
